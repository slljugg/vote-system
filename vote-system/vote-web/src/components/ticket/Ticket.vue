<template>
  <div id="ticket">
    ...
    <el-breadcrumb>
      ...
      <el-breadcrumb-item>所有投票</el-breadcrumb-item>
      <el-breadcrumb-item>所有用户的投票</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片试图区 -->
    <el-card>
      <el-table :data="ticketlist" :border="true">
        ...
        <el-table-column label="操作" width="160">
          <template slot-scope="scope">
            <!-- <el-button type="danger" icon="el-icon-delete"
              size="mini" @click="deleteticket(scope.row.id)">删除</el-button> -->
            <el-button type="info" icon="el-icon-more"
            size="mini" @click="showticketDetail(scope.row.id)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="ticketlogVisible = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getTicketInfo() {
      const { data: res } = await api.get('/ticketsinfo')
      if (res.meta.status !== 200) {
        return this.$message.error('投票数据请求失败')
      }
      console.log(res)
